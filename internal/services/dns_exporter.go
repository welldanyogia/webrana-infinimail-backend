...
// ExportBIND exports DNS records in BIND zone file format
// This format is compatible with Cloudflare zone file import
func (e *dnsExporter) ExportBIND(guide *DNSGuide, domain string) string {
	var sb strings.Builder

	// Cloudflare-compatible BIND format (no $TTL directive, TTL per record)
	// MX Record - use @ for root domain
	sb.WriteString(fmt.Sprintf("@\t%d\tIN\tMX\t%d\t%s.\n",
		guide.MXRecord.TTL,
		guide.MXRecord.Priority,
		guide.MXRecord.Value))

	// A Record - extract subdomain from full hostname
	aName := guide.ARecord.Name
	if strings.HasSuffix(aName, "."+domain) {
		aName = strings.TrimSuffix(aName, "."+domain)
	} else if aName == domain {
		aName = "@"
	}
	sb.WriteString(fmt.Sprintf("%s\t%d\tIN\tA\t%s\n",
		aName,
		guide.ARecord.TTL,
		guide.ARecord.Value))

	// TXT Record - extract subdomain from full hostname
	txtName := guide.TXTRecord.Name
	if strings.HasSuffix(txtName, "."+domain) {
		txtName = strings.TrimSuffix(txtName, "."+domain)
	} else if txtName == domain {
		txtName = "@"
	}
	sb.WriteString(fmt.Sprintf("%s\t%d\tIN\tTXT\t\"%s\"\n",
		txtName,
		guide.TXTRecord.TTL,
