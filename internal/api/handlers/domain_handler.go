...
func (h *DomainHandler) Create(c echo.Context) error {
	...
	if err := h.repo.Create(c.Request().Context(), domain); err != nil {
		if errors.Is(err, repository.ErrDuplicateEntry) {
			return api.Conflict(c, "domain already exists")
		}
		return api.InternalError(c, "failed to create domain")
	}

	return api.Created(c, domain)
}
...
func (h *DomainHandler) List(c echo.Context) error {
	...
	if err != nil {
		return api.InternalError(c, "failed to list domains")
	}

	return api.Success(c, domains)
}
...
func (h *DomainHandler) Get(c echo.Context) error {
	...
	if err != nil {
		return api.BadRequest(c, "invalid domain ID")
	}
...
func (h *DomainHandler) Update(c echo.Context) error {
	...
	if err := h.repo.Update(c.Request().Context(), domain); err != nil {
		if errors.Is(err, repository.ErrDuplicateEntry) {
			return api.Conflict(c, "domain name already exists")
		}
		return api.InternalError(c, "failed to update domain")
	}
...
func (h *DomainHandler) Delete(c echo.Context) error {
	...
	if err := h.repo.Delete(c.Request().Context(), uint(id)); err != nil {
		...
		return api.InternalError(c, "failed to delete domain")
	}
